...
CACHE_HIT_RATIO_GAUGE = Gauge("cache_hit_ratio", "Cache hit ratio", ["cache_type"])
...
CACHE_SIZE_GAUGE = Gauge("cache_size_bytes", "Cache size in bytes", ["cache_type"])

CACHE_EVICTIONS_COUNTER = Counter(
    "cache_evictions_total", "Total cache evictions", ["cache_type", "reason"]
)
...
class MemoryCache(BaseCache):
        ...
        async with self._lock:
            ...
            if entry is None:
                ...
                CACHE_OPERATIONS_COUNTER.labels(
                    operation="get", cache_type="memory", status="miss"
                ).inc()
                return None
            ...
            if entry.is_expired:
                ...
                CACHE_EVICTIONS_COUNTER.labels(
                    cache_type="memory", reason="expired"
                ).inc()
                return None
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            CACHE_OPERATIONS_COUNTER.labels(
                operation="set", cache_type="memory", status="error"
            ).inc()
            return False
    ...
    async def _evict_if_needed(self) -> None:
        ...
        while (
            ...
            or self.stats.total_size_bytes > self.config.max_size_mb * 1024 * 1024
        ):
            if not self._cache:
                break
            ...
            CACHE_EVICTIONS_COUNTER.labels(
                cache_type="memory", reason="size_limit"
            ).inc()
    ...
    async def _metrics_reporter(self) -> None:
        ...
        while True:
            try:
                ...
                CACHE_HIT_RATIO_GAUGE.labels(cache_type="memory").set(
                    stats["hit_ratio"]
                )
                CACHE_SIZE_GAUGE.labels(cache_type="memory").set(
...
class RedisCache(BaseCache):
        ...
        try:
            if self.config.redis_cluster_nodes:
                ...
                master = sentinel.master_for(
                    self.config.redis_sentinel_service, decode_responses=False
                )
                self.redis_client = aioredis.Redis(
                    connection_pool=master.connection_pool
                )
            else:
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            if data is None:
                self.stats.record_miss()
                CACHE_OPERATIONS_COUNTER.labels(
                    operation="get", cache_type="redis", status="miss"
                ).inc()
                return None
            ...
            CACHE_LATENCY_HISTOGRAM.labels(operation="get", cache_type="redis").observe(
            ...
            self.stats.record_error()
            CACHE_OPERATIONS_COUNTER.labels(
                operation="get", cache_type="redis", status="error"
            ).inc()
            return None
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            CACHE_LATENCY_HISTOGRAM.labels(operation="set", cache_type="redis").observe(
                time.time() - start_time
            )
            ...
            self.stats.record_error()
            CACHE_OPERATIONS_COUNTER.labels(
                operation="set", cache_type="redis", status="error"
            ).inc()
            return False
                ...
                if data is not None:
                    try:
                        value = CacheSerializer.deserialize(
                            data, self.config.serialization
                        )
                        results[key] = value
                        self.stats.record_hit()
                    except Exception as e:
            ...
            try:
                ...
                try:
                    item = await asyncio.wait_for(
                        self._write_behind_queue.get(), timeout=1.0
                    )
                    batch.append(item)
...
class CacheWarmer:
    ...
    async def warm_from_source(
        self, keys: List[str], source_func: Callable[[str], Any]
    ) -> int:
...
class CachingService:
    ...
    async def predict_and_warm(
        self, source_func: Callable[[str], Any], limit: int = 50
    ) -> int:
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        try:
            if (
                hasattr(self.cache.l2_cache, "redis_client")
                and self.cache.l2_cache.redis_client
            ):
...
def cache_key_for_llm_request(
    message: str, model: str, temperature: float, **kwargs
) -> str:
    ...
    return (
        f"llm:{hashlib.md5(json.dumps(key_data, sort_keys=True).encode()).hexdigest()}"
    )
