...
LLM_COST_COUNTER = Counter(
    "llm_cost_usd_total", "Total cost in USD", ["provider", "model"]
)

ACTIVE_REQUESTS_GAUGE = Gauge(
    "llm_active_requests", "Active LLM requests", ["provider"]
)
...
class OpenAIProvider(LLMProvider):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            logger.error(
                "OpenAI request failed", error=str(e), request_id=request.request_id
            )
            raise
    ...
    async def stream_generate(self, request: LLMRequest) -> AsyncIterator[str]:
        ...
        try:
            ...
            logger.error(
                "OpenAI streaming failed", error=str(e), request_id=request.request_id
            )
            raise
...
class AnthropicProvider(LLMProvider):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            logger.error(
                "Anthropic request failed", error=str(e), request_id=request.request_id
            )
            raise
...
class OllamaProvider(LLMProvider):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            logger.error(
                "Ollama request failed", error=str(e), request_id=request.request_id
            )
            raise
...
class LLMLoadBalancer:
    ...
    def select_provider(
        ...
        if model:
            candidates = [
                p for p in candidates if not p.config.models or model in p.config.models
            ]

        # Filter by health
        healthy_candidates = [
            p for p in candidates if p.is_healthy and p.config.enabled
        ]
    ...
    def record_request(self, provider: LLMProvider, success: bool = True) -> None:
        ...
        if not success:
            self.failure_counts[provider_key] = (
                self.failure_counts.get(provider_key, 0) + 1
            )
...
class LLMService:
    ...
    async def initialize(self) -> None:
        ...
        for provider in self.providers.values():
            try:
                ...
                logger.error(
                    f"Failed to initialize provider {provider.provider_type}: {e}"
                )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        if self.security_service:
            request.message = await self.security_service.sanitize_input(
                request.message
            )
        ...
        with ACTIVE_REQUESTS_GAUGE.labels(provider=provider.provider_type).track():
            try:
                ...
                LLM_LATENCY_HISTOGRAM.labels(
                    provider=provider.provider_type, model=response.model
                ).observe(response.latency_ms / 1000)

                LLM_TOKEN_COUNTER.labels(
                    provider=provider.provider_type, model=response.model, type="input"
                ).inc(response.usage["input_tokens"])

                LLM_TOKEN_COUNTER.labels(
                    provider=provider.provider_type, model=response.model, type="output"
                ...
                if self.security_service:
                    response.content = await self.security_service.sanitize_output(
                        response.content
                    )
