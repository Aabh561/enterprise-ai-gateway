...
PII_DETECTION_COUNTER = Counter(
    "pii_detections_total", "Total PII detections", ["entity_type", "action"]
)

CONTENT_FILTER_COUNTER = Counter(
    "content_filter_total", "Total content filtering events", ["filter_type", "action"]
)

SECURITY_LATENCY_HISTOGRAM = Histogram(
    "security_check_duration_seconds", "Security check duration", ["check_type"]
)
...
class SecurityConfig:
    ...
    pii_entities: List[PIIEntityType] = field(
        default_factory=lambda: list(PIIEntityType)
    )
...
class PIIProtectionService:
        ...
        try:
            ...
            for result in high_confidence_entities:
                ...
                confidence_scores[result.entity_type] = max(
                    confidence_scores.get(result.entity_type, 0.0), result.score
                )

                # Record metrics
                PII_DETECTION_COUNTER.labels(
                    entity_type=result.entity_type, action="detected"
                ).inc()
            ...
            SECURITY_LATENCY_HISTOGRAM.labels(check_type="pii_detection").observe(
    ...
    async def _anonymize_text(self, text: str, analysis_results) -> str:
        ...
        try:
            ...
            return await asyncio.get_event_loop().run_in_executor(None, anonymize_sync)
    ...
    async def sanitize_input(self, text: str) -> str:
        ...
        if result.found_pii:
            if result.anonymized_text:
                PII_DETECTION_COUNTER.labels(
                    entity_type="mixed", action="anonymized"
                ).inc()
...
class ContentFilter:
    ...
    async def filter_content(self, text: str) -> ContentFilterResult:
        ...
        if len(text) > self.config.max_input_length:
            reasons.append(
                f"Content too long ({len(text)} > {self.config.max_input_length})"
            )
            blocked = True
            risk_score += 0.3
        ...
        for i, pattern in enumerate(self._compiled_patterns):
            if pattern.search(text):
                ...
                CONTENT_FILTER_COUNTER.labels(
                    filter_type="blocked_pattern", action="blocked"
                ).inc()
        ...
        for i, pattern in enumerate(self._compiled_suspicious):
            if pattern.search(text):
                ...
                CONTENT_FILTER_COUNTER.labels(
                    filter_type="suspicious_pattern", action="detected"
                ).inc()
        ...
        SECURITY_LATENCY_HISTOGRAM.labels(check_type="content_filter").observe(
            time.time() - start_time
        )
...
class ThreatDetector:
    ...
    async def _is_suspicious_activity(
        self, content: str, user_id: Optional[str], source_ip: Optional[str]
    ) -> bool:
    ...
    def is_blocked(self, identifier: str) -> bool:
        ...
        return (
            identifier in self.suspicious_ips
            or self.failed_attempts.get(identifier, 0) > 10
        )
...
class SecurityService:
    ...
    async def _log_security_event(self, event: SecurityEvent) -> None:
        ...
        SECURITY_EVENTS_COUNTER.labels(
            event_type=event.event_type, severity=event.severity, action="logged"
        ).inc()
        ...
        if (
            ...
            and self.config.alert_on_critical
        ):
...
async def check_domain_reputation(domain: str) -> Dict[str, Any]:
    ...
    return {"safe": True, "category": "unknown", "risk_score": 0.0}
