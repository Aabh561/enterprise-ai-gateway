...
class AlertManager:
    ...
    async def _send_email_alert(self, alert: AlertEvent, rule: AlertRule) -> None:
        ...
        try:
            ...
            text = msg.as_string()
            server.sendmail(
                self.config.alert_email_from, self.config.alert_email_to, text
            )
            server.quit()
...
class BusinessMetricsCollector:
    ...
    def __init__(self, config: MonitoringConfig):
        self.config = config
        self.business_metrics: Dict[str, deque] = defaultdict(
...
class MonitoringService:
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        health["monitoring_tasks"] = (
            running_tasks > 0 if self._monitoring_tasks else True
        )
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                monitoring_service.finish_span(
                    span_id, tags={"error": str(e)}, status="error"
                )
                raise
            finally:
