// same script as portfolio build
async function loadConfig(){
  ...
  if(cfg.github_url||cfg.github_username){const url=cfg.github_url||`https://github.com/${cfg.github_username}`; cta.push(`<a class="btn" href="${url}" target="_blank" rel="noreferrer">GitHub</a>`);}  
  ...
  list.innerHTML=(cfg.projects||[]).map(p=>{
    ...
    return `<article class="card"><h3>${p.title} ${link}</h3><div class="meta-line">${p.dates||''}</div><p>${p.summary||''}</p><div class="badges">${tags}</div>${bullets?`<ul>${bullets}</ul>`:''}</article>`;
  }).join('');
  const sg=document.getElementById('skills-grid');
  sg.innerHTML = Object.entries(cfg.skills||{}).map(([k,vals])=>`<div class=\"skill-col\"><h4>${k}</h4><div class=\"skill-tags\">${vals.map(v=>`<span>${v}</span>`).join('')}</div></div>`).join('');
  const e=cfg.education||{};
  document.getElementById('education-block').innerHTML=`<div class=\"card\"><h3>${e.school||''}</h3><div class=\"meta-line\">${e.degree||''} ${e.gpa?`(GPA: ${e.gpa})`:''}</div><div class=\"meta-line\">${e.dates||''} · ${e.location||''}</div></div>`;
  ...
  if(cfg.github_url||cfg.github_username){const url=cfg.github_url||`https://github.com/${cfg.github_username}`;items.push(`<li><a href=\"${url}\" target=\"_blank\" rel=\"noreferrer\">GitHub</a></li>`);}
