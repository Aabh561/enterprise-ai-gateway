...
async def get_llm_service():
    ...
    if _llm_service is None:
        from services.llm_service import LLMService, create_openai_provider, create_ollama_provider
...
async def get_cache_service():
    ...
    if _cache_service is None:
        from services.caching_service import CachingService, CacheConfig
        config = CacheConfig(
            ttl_seconds=3600,
            max_size_mb=100,
...
async def get_vector_service():
    ...
    if _vector_service is None:
        from services.vector_service import VectorService, VectorDBConfig, EmbeddingConfig, ChunkingConfig
...
async def status(
    ...
    try:
        ...
        service_health["cache"] = "healthy" if cache_health.get("overall") else "degraded"
    except Exception:
    ...
    try:
        ...
        service_health["vector_db"] = "healthy" if vector_health.get("overall") else "degraded"
    except Exception:
...
async def chat_generate(
    ...
    try:
        ...
        if rag_context:
            ...
            system_prompt = create_rag_prompt(request.message, rag_context, system_prompt)
        ...
        for result in page_results:
            search_result = SearchResult(
                ...
                metadata={
                    ...
                    "document_type": result.chunk.metadata.document_type.value if result.chunk.metadata.document_type else None,
