...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            return JSONResponse(
                status_code=500,
                content={"error": {"code": "SECURITY_ERROR", "message": "Security check failed"}}
            )
    ...
    async def _check_security_headers(self, request: Request) -> None:
        ...
        if (not settings.app.debug and 
            settings.security.api_keys.require_https and 
            request.url.scheme != "https"):
            raise SecurityError(
                "HTTPS required",
                security_check="https_required"
            )
