...
class ChatRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(
        0.7, ge=0.0, le=2.0, description="Response creativity"
    )
    ...
    plugins: Optional[List[str]] = Field(
        None, description="Plugins to enable for this request"
    )
...
class ChatResponse(BaseModel):
    ...
    sources: Optional[List[Dict[str, Any]]] = Field(
        None, description="RAG sources used"
    )
    plugin_results: Optional[Dict[str, Any]] = Field(
        None, description="Plugin execution results"
    )
...
class DocumentIngestRequest(BaseModel):
    ...
    chunk_size: Optional[int] = Field(
        512, ge=100, le=2048, description="Text chunk size"
    )
    chunk_overlap: Optional[int] = Field(
        50, ge=0, le=500, description="Chunk overlap size"
    )
    metadata: Optional[Dict[str, Any]] = None
...
class SearchRequest(BaseModel):
    ...
    collection_name: Optional[str] = Field(
        "default", description="Collection to search"
    )
